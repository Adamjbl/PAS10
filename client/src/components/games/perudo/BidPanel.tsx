...
export function BidPanel({ onBid, minQuantity, currentBidFace, disabled, totalDice }: BidPanelProps) {
  const [quantity, setQuantity] = useState(minQuantity);
  const [face, setFace] = useState(currentBidFace || 2);

  // Update quantity when minQuantity changes
  useEffect(() => {
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              size="sm"
              variant="outline"
              onClick={() => setQuantity(Math.min(totalDice, quantity + 1))}
              disabled={disabled}
            >
              +
            </Button>
