            ...
            {players.map((player, index) => {
              ...
              return (
                <PlayerCardCircle
                  ...
                  onCardClick={(wireIndex) => onCutWire(player.id, wireIndex)}
                  canInteract={currentTurnPlayerId === currentPlayerId && player.id === currentPlayerId && !gameOver}
                />
              );
            })}
            ...
            >
              ...
              {unreadCount > 0 && (
                <motion.div
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  className="absolute -top-1 -right-1 w-6 h-6 bg-red-500 rounded-full flex items-center justify-center text-white text-xs border-2 border-slate-900"
                >
              ...
              >
                <div className="h-full relative">
                  <Button
                    ...
                    size="icon"
                    variant="ghost"
                    className="absolute -left-12 top-4 w-10 h-10 bg-slate-800/90 hover:bg-slate-700 text-white rounded-full shadow-lg z-10 border border-slate-600"
                  >
                    <X className="w-5 h-5" />
                  </Button>
                  <ChatPanel
